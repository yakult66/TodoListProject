...
<template>
    <div class="min-h-screen w-full p-4 md:p-10 lg:p-16 bg-linear-to-br from-[#eef2ff] to-[#f8fafc]">
        <div class="w-full max-w-[1000px] mx-auto bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] p-6 md:p-12 flex flex-col h-fit">
            <DataTable v-model:filters="filters" filterDisplay="menu" :value="taskDataShow" scrollable scrollHeight="flex" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" paginatorTemplate="RowsPerPageDropdown FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink"
                currentPageReportTemplate="第 {first} 頁 ｜ 共 {totalRecords} 頁" tableStyle="min-width: 50rem" :globalFilterFields="['title', 'content', 'assignedTo', 'createUser']">
                <template #header>
