...
<script setup lang="ts">
  ...
  import type { user as User, taskInterface} from '@/component/type';
  import { FilterMatchMode } from '@primevue/core';
  import Button from 'primevue/button';
  import InputText from 'primevue/inputtext';

  const reciveProps=defineProps({
    historyTask:{
   ...
   //篩選歷史任務（呈現在畫面上）
  const taskDataShow=computed(()=>{
    return reciveProps.historyTask!.filter(task=>task.createUser===reciveProps.loginUser!.account || task.assignedTo===reciveProps.loginUser!.name)})

  //計算頁數
  const rows = ref(5);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const totalPages = computed(() => {
      return taskDataShow.value? Math.ceil(taskDataShow.value.length / rows.value) : '0' ;
  });
