<template>
    <div class="w-full max-w-[1000px] mx-auto bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] p-6 md:p-12 flex flex-col h-fit">
        <DataTable :value="users" scrollable scrollHeight="flex" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" paginatorTemplate="RowsPerPageDropdown FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink"
                ...
                <Column field="status" sortable header="狀態" style="min-width: 120px">
                    <template #body="slotProps">
                         <span v-if="slotProps.data.status" class="text-[12px] text-indigo-500 bg-indigo-50 px-2.5 py-1 rounded-lg font-bold">
                ...
                <Teleport to="body" class="z-50 ">
                  <accEdit v-if="showEditDialog" @close="showEditDialog=false" @save="editUser" :user="user" :showEditDialog="showEditDialog" />
                </Teleport>
          </DataTable>
    </div>
</template>
...
<script setup lang="ts">
  ...
  const editUser = (id:number,editUser:user) => {
    showEditDialog.value=true;
    const User = receivedProps.users.find(user=>user.id===id);
    if(User){
